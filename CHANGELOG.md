...
- 「皮肤常驻」开关改为本地乐观生效，后台在 Windows 安全时间窗口内静默保存；后台握手期间不再显示“后台控制器未确认”、不回退开关，也不阻塞主题切换，真实业务拒绝仍会恢复并提示。
- 主题中心取消原始上传图片的 8 MiB 文件大小限制；大图仍会经过格式、尺寸和像素预算校验，并自动缩放压缩为受控 WebP 后再写入启动器。
- 主题切换改为本地优先：点击后立即应用并写入本地选择，后台只静默保存；超时、拒绝或控制器不可达时不再等待确认、弹出失败提示或回退壁纸。
...
- 基于上游 5.4.6 重新整理改动，保留上游较新的用户主题上传、常驻控制器、计划任务、进程归属、锁与 PowerShell 生命周期实现，未继续沿用旧分支中重复或风险更高的后台实验改动。

### 测试

- 更新主题中心回归测试，覆盖后台超时、请求冲突、快速连续切换和重新注入均不回退本地选择。
- 新增深色外观推断和编辑器扩展 CLI 排除测试。
