...
- **自定义上传**：菜单里选「＋ 自定义图片 / 视频」直接上传本地素材；图片会自动按图片风格取色（主色、辅色、面板底色、文字色），MP4/WebM 会作为静音循环视频壁纸。图片保留 64 MiB 解码预算，视频没有固定文件大小上限。上传成功后写入本机用户主题库（`%APPDATA%\\HeiGeCodexSkinStudio\\themes` / macOS Application Support），成为正式用户主题，并记入启动器 `selectedThemeId` / `lastNonNativeThemeId`，与内置主题一样可在「皮肤常驻」开启时跨重启复现。再次上传会生成新正式主题（同名同图则幂等覆盖）；「我的主题」中的 × 可删除磁盘主题。只有旧版 `custom-upload` 是 renderer localStorage 本地兼容槽，仍可在原生态下显示；新上传不再以该快捷槽为权威存储。
