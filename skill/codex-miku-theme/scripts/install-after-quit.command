...
LABEL="com.heigeai.codex-miku-theme.install"
...
"$NODE_BIN" "$ROOT/payload/src/theme-patch.mjs" check
mkdir -p "$LOG_DIR"
...
rm -f "$PLIST_PATH"
/bin/zsh "$PLIST_WRITER" "$PLIST_PATH" "$LABEL" "$RUNNER" install "$PATCHER" "$LOG_PATH" "$NODE_BIN"
