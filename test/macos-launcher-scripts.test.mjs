...
test("Codex launcher action uses the version-bound 9341 route", async (t) => {
  ...
  assert.equal(
    await recorded(root, recorder, "launch-skin.command", ["5.5.6", "codex"]),
    "product=codex\nlauncher-apply --launcher-version 5.5.6 --port 9341\n",
  );
});
...
test("WorkBuddy launcher action stays on its isolated one-shot 9342 route", async (t) => {
  const { root, recorder } = await fixture(t);
  assert.equal(
    await recorded(root, recorder, "launch-skin.command", ["5.5.6", "workbuddy"]),
    "product=workbuddy\nlauncher-apply --launcher-version 5.5.6 --app workbuddy --port 9342\n",
  );
});
  ...
  test(`${script} rejects an unknown product before invoking Node`, async (t) => {
    ...
    await assert.rejects(
      execFileAsync(join(root, "scripts", script),
        script === "launch-skin.command" ? ["5.5.6", "other"] : ["other"], {
          env: { ...process.env, HEIGE_TEST_RECORD: recorder },
        }),
      /不支持的产品/,
    );
