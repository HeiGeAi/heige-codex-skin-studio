...
async function fixture(t) {
  ...
  for (const name of ["launch-skin.command", "launcher-state.command"]) {
...
test("Codex launcher action uses the version-bound 9341 route", async (t) => {
  ...
  assert.equal(
    await recorded(root, recorder, "launch-skin.command", ["5.5.7", "codex"]),
    "product=codex\nlauncher-apply --launcher-version 5.5.7 --port 9341\n",
  );
});
...
test("WorkBuddy launcher action stays on its isolated one-shot 9342 route", async (t) => {
  const { root, recorder } = await fixture(t);
  assert.equal(
    await recorded(root, recorder, "launch-skin.command", ["5.5.7", "workbuddy"]),
    "product=workbuddy\nlauncher-apply --launcher-version 5.5.7 --app workbuddy --port 9342\n",
  );
});
