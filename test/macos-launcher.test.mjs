...
async function populateRuntime(installRoot, version = "5.5.8") {
...
test("creates a Finder-visible schema 4 app with the native universal executable", async (t) => {
  ...
  assert.match(plist, /<key>CFBundleShortVersionString<\/key>\s*<string>5\.5\.8<\/string>/);
...
test("launcher participant rollback removes a newly published app when no app existed before", async (t) => {
  ...
  assert.equal(participant.afterVersion, "5.5.8");
  for (const field of [
...
test("launcher staging reads release inputs from source while binding the app to the stable target", async (t) => {
  ...
  const sourceRoot = join(root, "5.5.8 发布源码");
...
for (const legacySchema of [1, 2]) {
  test(`upgrades an attributed Schema ${legacySchema} bundle and moves its stable install root`, async (t) => {
    ...
    const legacyPlist = renderMacosLauncherPlist(installRoot)
      ...
      .replace("<string>5.5.8</string>", "<string>1.0</string>")
      .replace("<string>5.5.8</string>", "<string>1</string>")
      .replace(
    ...
    assert.match(upgradedPlist, /<string>5\.5\.8<\/string>/);
